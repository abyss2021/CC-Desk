...
async function readProvidersConfig(): Promise<Record<string, unknown>> {
  const raw = await fs.readFile(path.join(tmpDir, 'CC-Desk', 'providers.json'), 'utf-8')
  return JSON.parse(raw) as Record<string, unknown>
}
...
describe('ProviderService', () => {
  ...
  describe('addProvider', () => {
    ...
    test('adding a provider should not sync settings until activated', async () => {
      ...
      await expect(fs.readFile(path.join(tmpDir, 'CC-Desk', 'settings.json'), 'utf-8')).rejects.toThrow()
    })
