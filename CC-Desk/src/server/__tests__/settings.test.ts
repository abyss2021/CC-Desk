...
describe('Models API', () => {
  ...
  it('GET /api/models/current should prefer CC-Desk managed model over global user model when provider is active', async () => {
  ...
  it('PUT /api/models/current should persist to CC-Desk managed settings when provider is active', async () => {
    const settingsSvc = new SettingsService()
    const providerSvc = new ProviderService()
    const provider = await providerSvc.addProvider({
      presetId: 'zhipuglm',
