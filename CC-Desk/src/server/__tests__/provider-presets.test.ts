...
describe('provider presets API', () => {
  ...
  test('GET and PUT /api/providers/settings read and write CC-Desk settings.json', async () => {
    const initial = {
      env: {
    ...
    const updatedRaw = await fs.readFile(path.join(tmpDir, 'CC-Desk', 'settings.json'), 'utf-8')
