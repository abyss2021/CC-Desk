...
export class ConversationService {
  ...
  private async buildChildEnv(
    ...
    // from ~/.claude/CC-Desk/settings.json instead of stale process.env.
    //
    ...
    return {
      ...
      PWD: workDir,
      ...(sdkUrl
        ? { CC_HAHA_COMPUTER_USE_HOST_BUNDLE_ID: 'com.cc-desk.desktop' }
        : {}),
      ...(desktopServerUrl
      ...
      ...(explicitProviderEnv
        ...
        : {}),
      // "官方" 模式 (CC-Desk/settings.json 没 provider env) 下,把 CLI 标记为
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const ccHahaDir = path.join(configDir, 'CC-Desk')
   ...
   * 强制切 OAuth。
   *
   * 默认 (读不到 settings.json) 按"官方"处理 — 即使用户从未用过 CC-Desk
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    const configDir =
      process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
    const settingsPath = path.join(configDir, 'CC-Desk', 'settings.json')
    try {
      const raw = fs.readFileSync(settingsPath, 'utf-8')
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
