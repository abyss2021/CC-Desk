 ...
 *
 * Storage: ~/.claude/CC-Desk/providers.json (lightweight index)
 * Active provider env vars written to ~/.claude/CC-Desk/settings.json
...
export class ProviderService {
  ...
  private getCcHahaDir(): string {
    return path.join(this.getConfigDir(), 'CC-Desk')
  }
   ...
   *  1. A CC-Desk provider is active → has auth
  ...
  async checkAuthStatus(): Promise<{
    hasAuth: boolean
    source: 'CC-Desk-provider' | 'original-settings' | 'env' | 'none'
    activeProvider?: string
  }> {
    // 1. Check CC-Desk active provider
    const index = await this.readIndex()
    if (index.activeId) {
      const provider = index.providers.find(p => p.id === index.activeId)
      if (provider?.apiKey) {
        return { hasAuth: true, source: 'CC-Desk-provider', activeProvider: provider.name }
      }
    }
