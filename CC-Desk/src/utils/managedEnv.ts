...
/**
 * Read env vars from ~/.claude/CC-Desk/settings.json (CC-Desk-specific provider
...
function getCcHahaSettingsEnv(): Record<string, string> {
  try {
    const ccDeskSettings = join(getClaudeConfigHomeDir(), 'CC-Desk', 'settings.json')
    const raw = readFileSync(ccDeskSettings, 'utf-8')
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // CC-Desk provider isolation: apply env from ~/.claude/CC-Desk/settings.json
  // AFTER userSettings so CC-Desk-specific provider config takes priority over
  // the original Claude Code's settings. This prevents CC-Desk from polluting
...
export function applyConfigEnvironmentVariables(): void {
  ...
  // CC-Desk provider isolation: same as in applySafeConfigEnvironmentVariables,
  // apply CC-Desk-specific env last so it overrides the original settings.
  Object.assign(process.env, filterSettingsEnv(getCcHahaSettingsEnv()))
