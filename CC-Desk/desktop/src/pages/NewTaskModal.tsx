...
export default function NewTaskModal() {
  ...
  return (
    <div className="bg-[var(--color-background)] text-[var(--color-on-surface)] font-[var(--font-body)] min-h-screen flex flex-col">
      {/* ─── TopAppBar / Header ──────────────────────────────────── */}
      <header className="bg-[#F8FAFC] flex justify-between items-center px-6 h-12 w-full z-40">
        <div className="flex items-center gap-6">
          <span className="text-sm font-bold text-[#0F172A] uppercase tracking-tighter font-[var(--font-headline)]">
            Claude Code Companion
          </span>
          <nav className="hidden md:flex gap-4 font-[var(--font-headline)] font-semibold tracking-wide text-sm">
            <a className="text-[#64748B] hover:text-[#2563EB] transition-colors cursor-pointer">Code</a>
            <a className="text-[#64748B] hover:text-[#2563EB] transition-colors cursor-pointer">Terminal</a>
            <a className="text-[#64748B] hover:text-[#2563EB] transition-colors cursor-pointer">History</a>
          </nav>
        </div>
        <div className="flex items-center gap-4">
          ...
          <span className="font-[var(--font-headline)] font-semibold tracking-wide text-sm text-[#64748B] cursor-pointer hover:text-[#2563EB] transition-colors">
            Settings
          </span>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        {/* ── Sidebar (dimmed) ────────────────────────────────────── */}
        <aside className="hidden md:flex flex-col p-4 gap-2 bg-[#F1F5F9] h-full w-[280px] opacity-30 pointer-events-none">
          {/* project selector */}
          ...
          <div className="font-[var(--font-body)] text-sm font-medium space-y-1">
            <div className="flex items-center gap-3 px-3 py-2 text-[#64748B]">
              <span className="material-symbols-outlined">add</span>New session
            </div>
            <div className="flex items-center gap-3 px-3 py-2 bg-[#F8FAFC] text-[#0F172A] rounded-lg relative before:content-[''] before:absolute before:left-[-8px] before:w-1 before:h-4 before:bg-[#2563EB] before:rounded-full">
            ...
            <div className="flex items-center gap-3 px-3 py-2 text-[#64748B]">
              ...
              {mockSessions.today.length > 0 && (
                <span className="ml-auto text-[10px] text-[#64748B]">{mockSessions.today.length}</span>
              )}
            </div>
            <div className="flex items-center gap-3 px-3 py-2 text-[#64748B]">
              <span className="material-symbols-outlined">event_note</span>Previous 7 Days
              {mockSessions.previous7Days.length > 0 && (
                <span className="ml-auto text-[10px] text-[#64748B]">{mockSessions.previous7Days.length}</span>
              )}
            </div>
          </div>
        </aside>
      ...
      {/* ─── Footer / Status Bar ─────────────────────────────────── */}
      <footer className="bg-[#F8FAFC] flex items-center justify-between px-4 z-50 fixed bottom-0 left-0 w-full h-8 border-t border-[#64748B]/20 font-[var(--font-body)] text-xs tracking-tight">
        <div className="flex items-center gap-3">
          <span className="text-[#64748B]">
            {mockStatusBar.user} &bull; {mockStatusBar.username} &bull; {mockStatusBar.plan}
          </span>
          <div className="flex items-center gap-4 ml-4">
            <span className="text-[#64748B] hover:bg-[#F1F5F9] px-2 py-0.5 rounded cursor-pointer transition-colors">
        ...
        <div className="flex items-center gap-4">
          <span className="text-[#2563EB] font-bold cursor-pointer">
