...
APP_BUNDLE_NAME="CC-Desk.app"
APP_BUNDLE_ID="com.cc-desk.desktop"

usage() {
  cat <<'EOF'
Build CC-Desk desktop for macOS Apple Silicon and output a DMG.
...
build_canonical_dmg() {
  ...
  staging_dir="$(mktemp -d "${TMPDIR:-/tmp}/CC-Desk-dmg.XXXXXX")"
  rw_dmg="$(mktemp "${TMPDIR:-/tmp}/CC-Desk-rw.XXXXXX").dmg"
...
if [[ -n "${LATEST_APP}" ]]; then
  ...
  build_canonical_dmg \
    ...
    "${CANONICAL_OUTPUT_DIR}/$(basename "${LATEST_DMG:-CC-Desk_0.1.0_aarch64.dmg}")"
fi
