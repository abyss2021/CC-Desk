...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/cc-desk',
      realPath: '/Users/dev/cc-desk',
      projectName: 'cc-desk',
      isGit: true,
      repoName: 'cc-desk',
      branch: 'main',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**cc-desk**')
    ...
    expect(info[1].content).toContain('cc-desk')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/cc-desk')
    expect(btn1.value.projectName).toBe('cc-desk')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/cc-desk')
    expect(event.action.value.projectName).toBe('cc-desk')
  })
